...
public abstract class IntervalsWithLockTestsBase : IAsyncDisposable {
    ...
    protected void Init(Func<IServiceCollection, IServiceCollection> lockServiceRegistration) {
    ...
    private async Task SaveInterval(TimeSpan? waitTime = null, TimeSpan? workTime = null, string key = null,
        ...
        using var lockAsync = await _distributedLock.CreateLockAsync(key ?? _key, TimeSpan.FromMinutes(5),
            waitTime ?? TimeSpan.FromMinutes(5), TimeSpan.FromMilliseconds(100), cancellationToken);
    ...
    protected IEnumerable<(Interval, Interval)> GetIntersections() {
        lock (_intervalsLock) {
            return Intervals.SelectMany(interval1 =>
                Intervals.Where(interval1.NotEquals).Where(interval1.Intersect)
                    .Select(interval2 => (interval1, interval2))).ToArray();
        }
    }
}
